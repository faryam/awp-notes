...
class Hero{
   ...
   function readCounters($id){
       ...
       $query = "(SELECT
                        id,local_name
                FROM
                    hero JOIN (SELECT hero_id FROM hero_strengths JOIN (SELECT this FROM strength_counters JOIN (SELECT strength_id FROM strengths JOIN hero_strengths ON strength_id=strength and hero_id=".$id.") s ON that=s.strength_id) k ON strength=k.this WHERE hero_id!=".$id." GROUP by hero_id) h
               ON
                   id=h.hero_id) 
                   UNION
   ...
   function readBestPicks($ids){
      ...
      for($x = 0; $x < count($ids); $x++)
      {
       ...
       $query = "(SELECT
                ...
                FROM
                    hero JOIN (SELECT hero_id FROM hero_strengths JOIN (SELECT this FROM strength_counters JOIN (SELECT strength_id FROM strengths JOIN hero_strengths ON strength_id=strength and (".$on.")) s ON that=s.strength_id) k ON strength=k.this WHERE ".$where." GROUP by hero_id) h
               ON
                   id=h.hero_id)
                   UNION
